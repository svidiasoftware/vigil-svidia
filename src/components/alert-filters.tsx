...
export function AlertFilters({ value, onChange }: AlertFiltersProps) {
  ...
  const hasFilters = value.cameras.length > 0 || value.severities.length > 0 || value.ackFilter !== "all" || value.starFilter !== "all" || value.agentFilter !== "all" || value.analyzerModel !== null || value.analyzerHost !== null || value.eventStatus !== null;
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      {/* Agent highlighted filter */}
      <button
        onClick={() => onChange({ ...value, agentFilter: value.agentFilter === "all" ? "highlighted" : "all" })}
        className={cn(
          "flex items-center gap-1 rounded-full px-2 py-1 text-[10px] font-medium transition-all h-8",
          value.agentFilter === "highlighted"
            ? "bg-yellow-500/20 text-yellow-400"
            : "bg-muted/30 text-muted-foreground/50 hover:text-muted-foreground",
        )}
        title="Show only alerts the agent flagged as interesting"
      >
      ...
      {hasFilters && (
        <Button
          ...
          onClick={() => onChange({ cameras: [], severities: [], sortBy: "captured_at", ackFilter: "all", starFilter: "all", agentFilter: "all", analyzerModel: null, analyzerHost: null, eventStatus: null })}
        >
          Clear
        </Button>
      )}
