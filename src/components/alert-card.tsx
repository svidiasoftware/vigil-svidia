...
interface AlertCardProps {
  ...
  onStar?: (alertId: string, starred: boolean) => void;
  onDelete?: (alertId: string) => void;
  isAdmin?: boolean;
}

export function AlertCard({ alert, acknowledged, onAcknowledge, onFilterCamera, onFalsePositive, onStar, onDelete, isAdmin }: AlertCardProps) {
      ...
      <div className="flex flex-1 flex-col justify-between min-w-0">
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {(alert.analyzer_host || alert.analyzer_model) && (
              <span
                className="text-muted-foreground/60 hover:text-muted-foreground transition-colors cursor-help"
                title={`Host: ${alert.analyzer_host || "—"}\nModel: ${alert.analyzer_model || "—"}`}
              >
            ...
            {alert.starred && (
              <svg className="h-3.5 w-3.5 text-yellow-400 fill-yellow-400" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
        ...
        {onStar && (
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
              onStar(alert.id, !alert.starred);
            }}
            className={`rounded-md p-1.5 transition-colors ${alert.starred ? "text-yellow-400 opacity-100" : "text-muted-foreground hover:text-yellow-400"}`}
            title={alert.starred ? "Unstar" : "Star (protect from deletion)"}
          >
            <svg className={`h-4 w-4 ${alert.starred ? "fill-yellow-400" : ""}`} viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" fill="none">
              <path strokeLinecap="round" strokeLinejoin="round" d="M11.48 3.499a.562.562 0 0 1 1.04 0l2.125 5.111a.563.563 0 0 0 .475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 0 0-.182.557l1.285 5.385a.562.562 0 0 1-.84.61l-4.725-2.885a.562.562 0 0 0-.586 0L6.982 20.54a.562.562 0 0 1-.84-.61l1.285-5.386a.562.562 0 0 0-.182-.557l-4.204-3.602a.562.562 0 0 1 .321-.988l5.518-.442a.563.563 0 0 0 .475-.345L11.48 3.5Z" />
            </svg>
          </button>
        )}
